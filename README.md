...
- **Admin routes are guarded twice**: once in `proxy.ts` (redirects unauthenticated/non-admin users before the page renders) and again in `app/admin/(protected)/layout.tsx` (re-verifies server-side, since the proxy layer alone is not a security boundary to rely on).
- **Admin user management** (`/admin/users`) is self-service — create admins, reset passwords, disable, or delete accounts entirely from the dashboard, no database access needed. It goes through the Supabase Auth Admin API (`lib/supabase/admin.ts`, service-role key), with every mutation re-checking `is_admin()` itself since the service-role client bypasses RLS. The last active admin can never be disabled or deleted, and nobody can disable/delete their own account from this page.
